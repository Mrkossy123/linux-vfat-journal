...
size_t strlen(const char *s);//Protypo synarthshs strlen xrhsh ws 3o orisma ths write
			    ...
			    struct msdos_dir_slot *slots, int *nr_slots)
{
	...
	*nr_slots = 0;
	...
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_build_slots(...) defined in namei_vfat.c\n");
	...
	*nr_slots = usize / 13;
	...
	//Pedia start,id,attr mesw pointer ps
	sprintf(temp,"Struct msdos_dir_slot(Defined in header file msdos_fs.h)\nFunction vfat_build_slots(...) defined in namei_vfat.c file\nField 1:start(Type:__le16 AKA unsigned short)value:%hu\nField 2:id(Type:__u8 AKA unsigned char)value:%u\nField 3:attr(Type:__u8 AKA unsigned char)value:%u\n\n\n\n",ps->start,ps->id,ps->attr);
...
shortname:
	...
	//Pedia size time date ctime_cs cdate mesw deikth de
...
out_free:
	__putname(uname);
	kfree(temp);//Apodesmeush bytes tou pinaka temp
	return err;
}
				  ...
				  unsigned int flags)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_lookup defined in namei_vfat.c and struct inode_operations vfat_dir_inode_operations(for VFAT file system)\n");
		       ...
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	struct timespec64 ts;
	int err;
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_create defined in namei_vfat.c and struct inode_operations vfat_dir_inode_operations(for VFAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_rmdir defined in namei_vfat.c and struct inode_operations vfat_dir_inode_operations(for VFAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
...
static int vfat_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_unlink defined in namei_vfat.c and struct inode_operations vfat_dir_inode_operations(for VFAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
		      ...
		      struct dentry *dentry, umode_t mode)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_mkdir defined in namei_vfat.c and struct inode_operations vfat_dir_inode_operations(for VFAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
		       ...
		       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_rename(...) defined in namei_vfat.c\n");
				...
				struct inode *new_dir, struct dentry *new_dentry)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: vfat_rename_exchange(...) defined in namei_vfat.c\n");
...
{	printk(KERN_INFO "EXECUTED FUNCTION: vfat_rename2 defined in namei_vfat.c and struct inode_operations vfat_dir_inode_operations(for VFAT file system)\n");
