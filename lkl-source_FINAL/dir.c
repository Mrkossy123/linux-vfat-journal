				     ...
				     sector_t phys)
{
	...
	sprintf(temp,"Struct msdos_sb_info(Defined in fat.h header file)\nFunction fat_dir_readahead(...) dir.c file\nField 1:sec_per_clus(Type:unsigned short) value:%hu\n\n\n",sbi->sec_per_clus);
			  ...
			  struct buffer_head **bh, struct msdos_dir_entry **de)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat__get_entry(...) defined in dir.c\n");
			  ...
			  wchar_t **unicode, unsigned char *nr_slots)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat_parse_long(...) defined in dir.c\n");
...
found:
	...
	sprintf(temp,"Struct fat_slot_info\nFunction fat_search_long(...) defined in dir.c file\nField 1: i_pos(Data type:loff_t AKA long long) Value:%lld\nField 2:slot_off(Data type:loff_t AKA long long) Value:%lld\nField 3:nr_slots(Data type:Integer) Value:%d\n\n\n",sinfo->i_pos,sinfo->slot_off,sinfo->nr_slots);
	write(fd,temp,strlen(temp));

	kfree(temp);//Apodesmeush bytes pou desmeuthkan gia to temp
	     ...
	     struct fat_slot_info *sinfo)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat_scan(...) defined in dir.c\n");
	...
	while (fat_get_short_entry(dir, &sinfo->slot_off, &sinfo->bh,
		...
		if (!strncmp(sinfo->de->name, name, MSDOS_NAME)) {
			...
			sprintf(temp,"Struct fat_slot_info\nFunction fat_scan(...) defined in dir.c file\nField 1: i_pos(Data type:loff_t AKA long long) Value:%lld\nField 2:slot_off(Data type:loff_t AKA long long) Value:%lld\nField 3:nr_slots(Data type:Integer) Value:%d\n\n\n",sinfo->i_pos,sinfo->slot_off,sinfo->nr_slots);
			write(fd,temp,strlen(temp));
...
{	char temp[500];
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat_scan_logstart(...) defined in dir.c\n");
	...
	while (fat_get_short_entry(dir, &sinfo->slot_off, &sinfo->bh,
		...
		if (fat_get_start(MSDOS_SB(sb), sinfo->de) == i_logstart) {
			...
			sprintf(temp,"Struct fat_slot_info\nFunction fat_scan_longstart(...) defined in dir.c file\nField 1: i_pos(Data type:loff_t AKA long long) Value:%lld\nField 2:slot_off(Data type:loff_t AKA long long) Value:%lld\nField 3:nr_slots(Data type:Integer) Value:%d\n\n\n",sinfo->i_pos,sinfo->slot_off,sinfo->nr_slots);
			write(fd,temp,strlen(temp));
...
int fat_remove_entries(struct inode *dir, struct fat_slot_info *sinfo)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat_remove_entries(...) defined in dir.c\n");
	...
	sprintf(temp,"Struct fat_slot_info\nFunction fat_remove_entries(...) defined in dir.c file\nField 1: i_pos(Data type:loff_t AKA long long) Value:%lld\nField 2:slot_off(Data type:loff_t AKA long long) Value:%lld\nField 3:nr_slots(Data type:Integer) Value:%d\n\n\n",sinfo->i_pos,sinfo->slot_off,sinfo->nr_slots);
	write(fd,temp,strlen(temp));
			       ...
			       struct buffer_head **bh, loff_t *i_pos)
{
	...
	*nr_cluster = (size + (sbi->cluster_size - 1)) >> sbi->cluster_bits;
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat_add_new_entries(...) defined in dir.c\n");
	...
	sprintf(temp,"Struct msdos_sb_info defined in fat.h header file\nFunction fat_add_new_entries(...)\nField 1:cluster_size(Type:unsigned int)Value:%u\nField 2:sec_per_clus(Type:unsigned short)Value:%hu\n\n\n",sbi->cluster_size,sbi->sec_per_clus);
	write(fd,temp,strlen(temp));
		    ...
		    struct fat_slot_info *sinfo)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: fat_add_entries(...) defined in dir.c\n");
...
found:
	...
	sprintf(temp,"Struct fat_slot_info\nFunction fat_add_entries(...) defined in dir.c file\nField 1: i_pos(Data type:loff_t AKA long long) Value:%lld\nField 2:slot_off(Data type:loff_t AKA long long) Value:%lld\nField 3:nr_slots(Data type:Integer) Value:%d\n\n\n",sinfo->i_pos,sinfo->slot_off,sinfo->nr_slots);
	write(fd,temp,strlen(temp));
