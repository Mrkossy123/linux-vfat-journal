				   ...
				   unsigned int flags)
{
	...
	struct inode *inode;
	int err;
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_lookup defined in namei_msdos.c and struct inode_operations msdos_dir_inode_operations(Directory operations for FAT file system)\n");
			   ...
			   struct timespec64 *ts, struct fat_slot_info *sinfo)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_add_entry(...) defined in namei_msdos.c\n");
			...
			struct dentry *dentry, umode_t mode, bool excl)
{
	...
	unsigned char msdos_name[MSDOS_NAME];
	int err, is_hid;
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_create defined in namei_msdos.c and struct inode_operations msdos_dir_inode_operations(Directory operations for FAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
...
static int msdos_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_rmdir defined in namei_msdos.c and struct inode_operations msdos_dir_inode_operations(Directory operations for FAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
		       ...
		       struct dentry *dentry, umode_t mode)
{
	...
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_mkdir defined in namei_msdos.c and struct inode_operations msdos_dir_inode_operations(Directory operations for FAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
...
static int msdos_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	struct fat_slot_info sinfo;
	int err;
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_unlink defined in namei_msdos.c and struct inode_operations msdos_dir_inode_operations(Directory operations for FAT file system)\n");
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	err = msdos_find(dir, dentry->d_name.name, dentry->d_name.len, &sinfo);
	if (err)
		goto out;
			...
			unsigned int flags)
{
	...
	int err, is_hid;
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_rename defined in namei_msdos.c and struct inode_operations msdos_dir_inode_operations(Directory operations for FAT file system)\n");
...
{	
	printk(KERN_INFO "EXECUTED FUNCTION: msdos_fill_super(...) defined in namei_msdos.c\n");
